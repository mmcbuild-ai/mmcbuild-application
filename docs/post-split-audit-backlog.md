...
- ✅ **Login submit loading state** (was: no spinner) — DONE (`6e9b5a3`-area). The button already disabled + swapped text; upgraded to an animated `Loader2` spinner on both Sign In + Send Magic Link so the few-second server-action wait is unmistakable. The underlying `/login` cold-paint hydration (LCP ~3.9s) is still the perf-polish item above.
...
- ✅ **Brochure dropdowns 36px → 44px on mobile** — DONE. Bumped the shared `SelectTrigger` default to `h-11 md:h-10` (44px mobile / 40px desktop, matching the `Input` pattern) — fixes the contact-form country-code + Role selects *and* the waitlist / trades-supplier selects in one place.
